...
func (m *ModuleInstance) newBackends(config map[string]interface{}) *Backends {
	...
	for name, cfg := range config {
		...
		case "":
			conn := parseConn(cfg, defaults[""])
			if driver, err := postgres.New(conn); err == nil {
				b. = backends.NewK6Client(m.vu, driver, "")
				enabledContainers = append(enabledContainers, parseContainer(cfg, containers[""]))
				driver.CaptureConfig(ctx, "")
			}
