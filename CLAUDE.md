...
1. Each backend package calls `Register()` in its `init()` function (e.g., `backends/postgres/driver.go` registers "paradedb", "postgres-fts", "")
...
- The postgres driver (`backends/postgres/`) handles three separate backends (paradedb, postgres-fts, ) via different registrations with different default ports and env vars
